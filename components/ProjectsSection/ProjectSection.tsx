...
function ProjectsSction({ projectSection, projects }: Props) {
  ...
  useEffect(() => {
    ...
    client
      .fetch(
        '*[_type == "project"] {..., _id, title, tags ,author-> ,categories[]-> , categoryTag}'
      )
      .then((data: Project[]) => {
        setProject(data);
      });
  ...
  const filteredProjects = selectedTag
    ? project.filter((project) => project.tags.includes(selectedTag))
    : project;
            ...
            {filteredProjects.map((project) => (
              ...
              >
                ...
                <div className="relative ">
                  ...
                  <p
                    key={project?._id}
                    className={`absolute top-0  ${project?.tagTitleColor} font-semibold py-1 px-3 rounded-br-lg rounded-tl-lg ${project?.tagBgColor}`}
                  >
                  ...
                  >
                    <Image
                      src={urlFor(project?.author?.authorImage)
                        .width(500)
                        .height(500)
                        .url()}
                  ...
                  <div key={project?._id} className="mt-6 mb-6 ">
                    <span
                      className={`inline-flex items-center px-2  text-xs font-bold pointer-events-auto  ${project?.statusClassName}`}
                    >
                      <span className="mr-1.5 flex justify-center h-2 w-2 items-center ">
                        <span
                          className={`absolute flex  w-2 h-2 rounded-full  opacity-75 animate-ping ${project?.statusColor}`}
                        ></span>
                        <span
                          className={`relative inline-flex w-1 h-1 rounded-full ${project?.statusColor}`}
                        ></span>
                      </span>
                  ...
                  >
                    <div className="flex items-center">
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="font-mono">{project?.author?.name}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                >
                  <button
                    ...
                    rounded-lg shadow-lg hover:scale-105 ease-in duration-300 ${project?.buttonBgColor} ${project?.buttonHoverBgColor}  `}
                  >
          ...
          {projectSection?.map((pr) => (
            <>
              ...
              >
                <Image
                  key={pr?._id}
                  src={urlFor(pr?.tagEmptyPargraphIcon)
                    .width(500)
                    .height(500)
                    .url()}
                  alt={pr?.projectSectionTitle}
                  width={300}
                  height={300}
                />
              </motion.div>
