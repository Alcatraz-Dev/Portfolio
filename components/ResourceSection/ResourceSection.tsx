...
import { Resource, ResourceSection } from "@/typings";
...
function ResourcesSction({ resourceSection, resources }: Props) {
  ...
  useEffect(() => {
    ...
    client
      .fetch(
        '*[_type == "resource"] {..., _id, title, tags ,author-> ,categories[]-> , categoryTag}'
      )
      .then((data: Resource[]) => {
        setResource(data);
      });
  ...
  const handleTagClick = (links: string) => {
    setSelectedTag(links);
  };

  const filteredProjects = selectedTag
    ? resource.filter((resource) => resource.tags.includes(selectedTag))
    : resource;
    ...
    <section id="resources" className="p-5 sm:p-10 ">
      {resourceSection?.map((res) => (
        ...
        >
          ...
          >
            {res?.resourceSectionTitle}
          </h2>
          <p className="text-base lg:text-lg ">{res?.shortDescription}</p>
        </div>
      ))}
          ...
          {resourceSection?.map((filterTab) => (
            ...
            >
              {filterTab?.tabFilter?.map((linktab) => (
                <button
                  key={linktab?._id}
                  onClick={() => handleTagClick(linktab?.title)}
                  role="tab"
                  aria-selected="true"
                  aria-controls="All"
                  id="All"
                  tabIndex={0}
                  className={` ${
                    linktab?.title === selectedTag ? linktab?.tabColor : ""
                  } relative block  h-10 px-6 tab rounded-full`}
                >
                  <span className=" text-zinc-700 dark:text-zinc-300 capitalize font-bold text-xs">
                    {linktab?.title}
                  </span>
                </button>
              ))}
              ...
              >
                ...
                <div className="relative ">
                  ...
                  />
                     ...
                     <p
                    key={resource?._id}
                    className={`absolute left-[2px] top-0  ${resource?.tagTitleColor} font-semibold py-1 px-3 rounded-br-lg rounded-tl-lg ${resource?.tagBgColor}`}
                  >
                  ...
                  <div key={resource?._id} className="mt-6 mb-6 ">
                    <span
                      className={`inline-flex items-center px-2  text-xs font-bold pointer-events-auto  ${resource?.statusClassName}`}
                    >
                      <span className="mr-1.5 flex justify-center h-2 w-2 items-center ">
                        <span
                          className={`absolute flex  w-2 h-2 rounded-full  opacity-75 animate-ping ${resource?.statusColor}`}
                        ></span>
                        <span
                          className={`relative inline-flex w-1 h-1 rounded-full ${resource?.statusColor}`}
                        ></span>
                      </span>
                  ...
                  >
                    <div className="flex items-center">
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="font-mono">{resource?.author?.name}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <ClientSideRoute
                  key={resource?._id}
                  route={`/resource/${resource?.slug.current}`}
                >
                  <button
                    key={resource?._id}
                    className={`flex items-center justify-center right-0 p-2 mt-4 mb-4 text-center text-xs font-bold w-full text-white py-2 
                    rounded-lg shadow-lg hover:scale-105 ease-in duration-300 ${resource?.buttonBgColor} ${resource?.buttonHoverBgColor}  `}
                  >
                    ...
                    <Image
                      key={resource?._id}
                      src={urlFor(resource?.buttonIcon)
                        .width(500)
                        .height(500)
                        .url()}
                      alt={resource?.title}
          ...
          {resourceSection?.map((res) => (
            <>
              <motion.div
                ...
                animate={{ opacity: 1, scale: 1 }}
                transition={{ duration: 0.5 }}
                className="relative flex-1 flex justify-center items-center mt-10"
              >
                <Image
                  key={res?._id}
                  src={urlFor(res?.tagEmptyPargraphIcon)
                    .width(500)
                    .height(500)
                    .url()}
                  alt={res?.resourceSectionTitle}
                  width={300}
                  height={300}
                />
              </motion.div>
              <motion.h2
                ...
                animate={{ opacity: 1, scale: 1 }}
                transition={{ duration: 0.5 }}
                className={`text-2xl font-bold mb-4  text-center mt-10  text-gray-500 `}
              >
                {res?.tagEmptyPargraph}
              </motion.h2>
            </>
          ))}
        </div>
...
export default ResourcesSction;
