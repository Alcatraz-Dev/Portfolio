...
import { client } from "@/lib/sanity.client"; // You need to set up your Sanity client
...
function SearchBar() {
  ...
  const handleSearch = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (searchTerm) {
      setIsLoading(true);
      const query = groq`
      *[_type == "resource" &&  (title match $searchTerm || categoryTag[] match $searchTerm || tags[] match $searchTerm ) ]{
        ...,author->, 
        stacks[]->,
        paragraph[]->,
        customButton[]->,
      ...
      const searchResults = await client.fetch(query, { searchTerm });
  ...
  const handleSearchInputChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    // Adjust the timeout duration as neededtimeout duration as needed
  };

  return (
    <>
      <p className="text-gray-500 font-semibold mt-5 mb-0">
        Search resource by category , tag , title...
      </p>
          ...
          <div className="relative rounded-2xl backdrop-blur-xl bg-white/40 dark:bg-zinc-700/40 px-6 pt-10 pb-8 shadow-lg ring-1 ring-gray-900/5 ">
            <div ref={boxRef} className="mx-auto max-w-md">
              <form action="" className="relative mx-auto w-max">
                <input
                  type="search"
                  className=" peer cursor-pointer relative z-10 h-12 w-12 rounded-full  border bg-transparent pl-10 outline-none focus:w-full focus:cursor-text focus:border-purple-300 focus:pl-14 focus:pr-0"
              ...
              {showResults && (
                <ul>
                  {results.length === 0 && query === "" ? (
                    // Display all resource when no search input or results
                    <p className="mt-5 flex justify-center">
                      Display all resources here
                    </p>
                  ) : (
                    results.map((result, index) => (
                      <>
                        ...
                        >
                          <ClientSideRoute
                            key={result?._id}
                            route={`/resource/${result?.slug.current}`}
                          >
                            <button
                              key={result?._id}
