...
import { DefaultNotFoundPage } from "@/typings";
import Image from "next/image";
import { useEffect, useState } from "react";
import "../styles/globals.css";

type Props = {
  NotFoundPageData: DefaultNotFoundPage;
};
export const revalidate = 10;
  function NotFound() {
  const [pageData, setPageData] = useState<DefaultNotFoundPage>();

  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await client.fetch<DefaultNotFoundPage[]>(
          `*[_type == "defaultNotFoundPage"]`
        );
  ...
  if (!pageData) {
    return <div className=" w-full h-screen flex justify-center items-center text-3xl font-bold text-gray-800 dark:text-white">
      Loading...
      </div>; // Loading state
  }
    ...
    <Providers>
      <div className="flex items-center justify-center h-screen bg-gray-100 dark:bg-zinc-800">
        <div className="text-center">
          ...
          {pageData?.errorImage && (
            <div  className=" ml-36 mt-5 mb-5 flex justify-items-center">
            <Image
              src={urlFor(pageData?.errorImage).width(500).height(500).url()}
              width={250}
              height={250}
              alt="not found image"
          ...
          <p className="px-3 mt-4 mb-5 text-gray-600 dark:text-gray-300">
            {pageData?.description}
          </p>
          {pageData.url && (
            <a
              href={pageData?.url}
              className={`mt-10 px-4 py-2 ${pageData?.buttonBg} ${pageData?.buttonHoverBg} rounded-md`}
            >
             <span className={`${pageData?.buttonTitelClassName}`}>{pageData?.buttonTitel}</span>
            </a>
          )}
        </div>
      </div>
